...
function Result({ ans, onRetry, saving, setSaving }) {
  ...
  const save = async () => {
    ...
    if (bestLike) params.set("like", bestLike);
    if (bestKnow) params.set("know", bestKnow);
    if (worstLike) params.set("avoid", worstLike);
