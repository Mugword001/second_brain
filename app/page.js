...
// 自前カレンダー（Supabaseのplans）に保存する
async function savePlan(item) {
  if (!supabaseReady || !supabase) {
    return { ok: false, reason: "Supabaseに接続できていません" };
  }
  if (!item.due_date) {
    return { ok: false, reason: "日付が空です" };
  }
  const { error } = await supabase.from("plans").insert({
    title: item.subject || "（無題）",
    type: item.type || "予定",
    plan_date: item.due_date,
    note: item.note || null,
    emoji: emojiFor(item.type),
  });
...
function Review({ items, remaining, mode, addedIds, editingId, setEditingId, updateItem, removeItem, markAdded, reset }) {
  ...
  const addOne = async (it) => {
    setSavingId(it._id);
    setSaveError(null);
    const res = await savePlan(it);
    setSavingId(null);
    if (res.ok) {
      markAdded(it._id);
    } else {
      setSaveError(`「${it.subject || "予定"}」を保存できませんでした：${res.reason}`);
    }
  };
  ...
  const addAll = async () => {
    ...
    for (const it of remaining) {
      const res = await savePlan(it);
      if (res.ok) markAdded(it._id);
      else { setSaveError(`「${it.subject || "予定"}」を保存できませんでした：${res.reason}`); break; }
    }
  };
  ...
  return (
    <section style={{ marginTop: 16 }}>
      ...
      {saveError && (
        <div style={{ background: "#FBEDEA", border: `1px solid ${C.shuSoft}`, color: C.shu, borderRadius: 10, padding: "12px 14px", fontSize: 13, marginBottom: 14 }}>
          {saveError}
        </div>
      )}
      {items.length === 0 && (
          ...
          return (
            <div key={it._id} style={{ background: C.white, border: `1px solid ${C.line}`, borderRadius: 10, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 12px" }}>
                <button onClick={() => addOne(it)} disabled={savingId === it._id}
                  aria-label="カレンダーに追加"
                  style={{ flexShrink: 0, width: 30, height: 30, borderRadius: "50%", border: `2px solid ${C.ai}`, background: savingId === it._id ? C.washiDeep : "transparent", cursor: savingId === it._id ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
